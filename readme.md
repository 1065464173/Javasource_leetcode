...
虽然第二种方案相比与第一种时间复杂度相同，但是因为使用了递归，并且维护了四个信息的结构体，运行的时间略长，空间复杂度也不如方法一优秀，而且难以理解。但这种方法存在的意义是可以解决更多的问题，对于大规模的查询情况下
更有优势——线段树。所以有时候算法需要和业务相结合，真正开发中可能需要关注算法的可扩展性，并不是一时的效率快就是好。
